...
export const createColumns = ({ t, onEdit, onDelete }: WalletAddressActionsProps): ColumnDef<IWalletAddress>[] => [
  {
    accessorKey: 'label',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className='font-medium'>{formatAddress(address)}</div>;
    },
  },
  {
    accessorKey: 'blockchain',
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('table.headers.blockchain')} />,
    cell: ({ row }) => {
      const _row = row.original;
      const blockchain = _row.blockchain;
      return <div className='font-medium'>{capitalizeFirstLetter(blockchain)}</div>;
    },
  },
  {
    id: 'actions',
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align='end'>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => onEdit?.(_row)}>{t('table.actions.edit')}</DropdownMenuItem>
            <DropdownMenuItem onClick={() => onDelete?.(_row)} className='text-destructive'>
