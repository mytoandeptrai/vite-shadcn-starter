...
const MerchantAddressList = ({ onAdd, onEdit, isLoading }: MerchantAddressListProps) => {
  ...
  return (
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        ...
        <Button type='button' variant='outline' size='sm' onClick={onAdd} disabled={isLoading} className='gap-2'>
      ...
      <Show when={fields.length === 0}>
        <div className='rounded-lg border border-dashed p-8 text-center'>
          <p className='text-muted-foreground text-sm'>{t('messages.no-wallet-addresses')}</p>
        </div>
      </Show>
                ...
                <div key={field.id} className='rounded-lg border p-4'>
                  <div className='flex flex-col items-center justify-between gap-2 md:flex-row md:items-start md:gap-0'>
                    <div className='w-full space-y-2 md:flex-1'>
                      <div className='flex items-center gap-2'>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className='w-32 truncate font-medium text-sm md:w-56'>{field.label}</span>
                          </TooltipTrigger>
                          <TooltipContent>
                      ...
                      <p className='break-all font-mono text-muted-foreground text-xs'>{field.address}</p>
                    </div>
      ...
      <Show when={!!errors.walletAddresses}>
        <p className='mt-1.5 font-medium text-destructive text-sm'>{errors?.walletAddresses?.message}</p>
      </Show>
    </div>
  );
};

const MerchantFormUi = ({ actionType, isLoading, onClose, onAdd, onEdit }: MerchantFormUiProps) => {
  const { t } = useTranslation('merchants-page');
  const { form } = useMerchantFormContext();

  return (
    <div className='space-y-4'>
      <Show when={actionType === 'create'}>
        ...
        <MerchantAddressList onAdd={onAdd} onEdit={onEdit} isLoading={isLoading} />
      </Show>
