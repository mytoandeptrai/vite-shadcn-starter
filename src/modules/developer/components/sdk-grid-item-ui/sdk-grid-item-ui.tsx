...
const SDKGridItemUi = ({ sdk, onClick }: SDKGridItemUiProps) => {
  return (
    <Card
      className={cn('cursor-pointer transition-all hover:shadow-md hover:border-primary/50', 'group')}
      onClick={onClick}
    >
      <CardContent className='p-6'>
        <div className='flex items-start justify-between gap-4'>
          <div className='flex-1 min-w-0'>
            <h3 className='font-semibold text-lg mb-1 group-hover:text-primary transition-colors'>{sdk.name}</h3>
            {sdk.version && <p className='text-sm text-muted-foreground mb-2'>v{sdk.version}</p>}
